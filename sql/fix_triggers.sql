-- Fix or disable problematic triggers
-- This should be added to the all_fixes.sql file

-- Check if send_shipment_notification trigger exists and disable it if it does
DO $$
DECLARE
...
BEGIN
  ...
  -- If trigger exists, disable it
  IF trigger_exists THEN
    EXECUTE 'ALTER TABLE shipments DISABLE TRIGGER send_shipment_notification';
    
    -- Optionally create a fixed version of the trigger function
    ...
    EXECUTE 'ALTER TABLE shipments ENABLE TRIGGER send_shipment_notification';
  END IF;
END $$;
