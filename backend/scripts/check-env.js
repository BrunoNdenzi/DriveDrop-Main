...
try {
  ...
  console.log(
    `Node.js version: ${nodeVersionOutput} (required: v${requiredNodeVersion} or higher)`
  );
  console.log(
    `npm version: ${npmVersionOutput} (required: v${requiredNpmVersion} or higher)`
  );

  if (nodeVersion < requiredNodeVersion) {
    console.error(
      '\x1b[31m%s\x1b[0m',
      `Error: Node.js v${requiredNodeVersion} or higher is required.`
    );
    process.exit(1);
  }
  ...
  if (npmVersion < requiredNpmVersion) {
    console.error(
      '\x1b[31m%s\x1b[0m',
      `Error: npm v${requiredNpmVersion} or higher is required.`
    );
    process.exit(1);
  }
