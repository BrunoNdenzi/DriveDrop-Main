...
if (!process.env.SUPABASE_URL || !process.env.SUPABASE_SERVICE_ROLE_KEY) {
  console.error(
    '\x1b[31mError: Missing Supabase environment variables.\x1b[0m'
  );
  console.error(
...
async function createAdminUser() {
  try {
    ...
    if (profileQueryError) {
      console.warn(
        'Could not check profiles table:',
    ...
    if (existingProfiles && existingProfiles.length > 0) {
      console.log(
        `\x1b[33mUser ${EMAIL} already exists. Updating to admin role...\x1b[0m`
      );
      ...
      console.log(
        `\x1b[32mUser ${EMAIL} has been updated to admin role.\x1b[0m`
      );
      return;
    }
