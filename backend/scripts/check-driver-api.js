...
// Paths to check
const driverRoutesPath = path.join(__dirname, '..', 'src', 'routes', 'driver.routes.ts');
const appControllerPath = path.join(__dirname, '..', 'src', 'controllers', 'application.controller.ts');
const supabaseFunctionsPath = path.join(__dirname, '..', '..', 'supabase', 'migrations');
...
if (fs.existsSync(appControllerPath)) {
  ...
  if (controllerContent.includes('getDriverApplications')) {
    ...
    if (controllerContent.includes('get_driver_applications')) {
      console.log('ℹ️ Controller is using "get_driver_applications" database function');
      ...
      if (fs.existsSync(supabaseFunctionsPath)) {
        ...
        for (const file of migrationFiles) {
          if (file.endsWith('.sql')) {
            const content = fs.readFileSync(path.join(supabaseFunctionsPath, file), 'utf8');
            if (content.includes('get_driver_applications')) {
              console.log(`✅ Found get_driver_applications in migration file: ${file}`);
        ...
        if (!foundDbFunction) {
          console.log('❌ get_driver_applications database function NOT found in any migration file');
        }
      } else {
    ...
    console.log('❌ getDriverApplications function is NOT defined in controller');
  }
} else {
...
console.log('3. Make sure environment variables are correctly set in the mobile app');
console.log('4. If using a SQL function, ensure it\'s properly deployed to the database');
