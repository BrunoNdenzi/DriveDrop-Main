...
CREATE TABLE public.pricing_config (
  ...
  updated_by uuid,
  notes text,
  intelligence_min_confidence integer DEFAULT 50 CHECK (intelligence_min_confidence >= 0 AND intelligence_min_confidence <= 100),
  intelligence_min_data_quality text DEFAULT 'limited'::text CHECK (intelligence_min_data_quality = ANY (ARRAY['insufficient'::text, 'limited'::text, 'good'::text, 'excellent'::text])),
  historical_alignment_weight numeric DEFAULT 0.30 CHECK (historical_alignment_weight >= 0::numeric AND historical_alignment_weight <= 1::numeric),
  historical_deviation_threshold integer DEFAULT 15 CHECK (historical_deviation_threshold >= 0),
  historical_min_sample_size integer DEFAULT 20 CHECK (historical_min_sample_size >= 0),
  demand_premium_percent numeric DEFAULT 5.00 CHECK (demand_premium_percent >= 0::numeric),
  demand_discount_percent numeric DEFAULT 3.00 CHECK (demand_discount_percent >= 0::numeric),
  loyalty_discount_percent numeric DEFAULT 3.00 CHECK (loyalty_discount_percent >= 0::numeric),
  conversion_boost_percent numeric DEFAULT 5.00 CHECK (conversion_boost_percent >= 0::numeric),
  momentum_premium_percent numeric DEFAULT 3.00 CHECK (momentum_premium_percent >= 0::numeric),
  max_price_adjustment_percent numeric DEFAULT 20.00 CHECK (max_price_adjustment_percent >= 0::numeric),
...
CREATE TABLE public.driver_applications (
  ...
  updated_at timestamp with time zone NOT NULL DEFAULT now(),
  admin_comment text,
  mvr_check_status text,
  mvr_check_vendor text,
  mvr_check_id text,
  mvr_check_result jsonb,
  mvr_check_completed_at timestamp with time zone,
  mvr_violations_count integer DEFAULT 0,
  mvr_eligible boolean,
  mvr_ineligible_reasons ARRAY,
  dot_number text,
  dot_verified boolean DEFAULT false,
  dot_company_name text,
  dot_status text,
  dot_verified_at timestamp with time zone,
  fcra_disclosure_shown_at timestamp with time zone,
  fcra_consent_obtained_at timestamp with time zone,
  fcra_consent_ip_address inet,
  fcra_consent_signature text,
  adverse_action_pre_notice_sent_at timestamp with time zone,
  adverse_action_final_sent_at timestamp with time zone,
  mvr_report_copy_provided_at timestamp with time zone,
...
CREATE TABLE public.leads (
  id uuid NOT NULL DEFAULT uuid_generate_v4(),
  source text NOT NULL CHECK (source = ANY (ARRAY['fmcsa'::text, 'manual'::text, 'csv_import'::text, 'web_scrape'::text, 'referral'::text, 'website'::text, 'linkedin'::text, 'other'::text])),
  source_id text,
  lead_type text NOT NULL CHECK (lead_type = ANY (ARRAY['carrier'::text, 'broker'::text, 'dealer'::text, 'shipper'::text, 'fleet_owner'::text, 'auction_house'::text])),
  company_name text NOT NULL,
  dba_name text,
  ...
  status text NOT NULL DEFAULT 'new'::text CHECK (status = ANY (ARRAY['new'::text, 'contacted'::text, 'responding'::text, 'qualified'::text, 'negotiating'::text, 'converted'::text, 'lost'::text, 'unsubscribed'::text, 'invalid'::text])),
  ...
  CONSTRAINT leads_campaign_id_fkey FOREIGN KEY (campaign_id) REFERENCES public.outreach_campaigns(id)
);
CREATE TABLE public.outreach_campaigns (
  id uuid NOT NULL DEFAULT uuid_generate_v4(),
  name text NOT NULL,
  description text,
  campaign_type text NOT NULL CHECK (campaign_type = ANY (ARRAY['email'::text, 'sms'::text, 'phone'::text, 'multi_channel'::text])),
  target_lead_type text NOT NULL CHECK (target_lead_type = ANY (ARRAY['carrier'::text, 'broker'::text, 'dealer'::text, 'shipper'::text, 'fleet_owner'::text, 'auction_house'::text, 'all'::text])),
  email_subject text,
  email_body text,
  sms_body text,
  status text NOT NULL DEFAULT 'draft'::text CHECK (status = ANY (ARRAY['draft'::text, 'scheduled'::text, 'active'::text, 'paused'::text, 'completed'::text, 'cancelled'::text])),
...
CREATE TABLE public.outreach_log (
  ...
  campaign_id uuid,
  channel text NOT NULL CHECK (channel = ANY (ARRAY['email'::text, 'sms'::text, 'phone'::text, 'linkedin'::text, 'other'::text])),
  direction text NOT NULL DEFAULT 'outbound'::text CHECK (direction = ANY (ARRAY['outbound'::text, 'inbound'::text])),
  subject text,
  body text,
  status text NOT NULL DEFAULT 'sent'::text CHECK (status = ANY (ARRAY['pending'::text, 'dry_run'::text, 'sent'::text, 'delivered'::text, 'opened'::text, 'clicked'::text, 'replied'::text, 'bounced'::text, 'failed'::text, 'unsubscribed'::text])),
  response_text text,
  responded_at timestamp with time zone,
  sent_at timestamp with time zone DEFAULT now(),
  sent_by uuid,
  ...
  CONSTRAINT outreach_log_lead_id_fkey FOREIGN KEY (lead_id) REFERENCES public.leads(id),
  CONSTRAINT outreach_log_campaign_id_fkey FOREIGN KEY (campaign_id) REFERENCES public.outreach_campaigns(id)
);
...
CREATE TABLE public.driver_documents (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  driver_id uuid NOT NULL,
  document_type text NOT NULL CHECK (document_type = ANY (ARRAY['license'::text, 'insurance'::text, 'registration'::text, 'background_check'::text, 'other'::text])),
  file_url text NOT NULL,
  file_name text NOT NULL,
  ...
  CONSTRAINT driver_documents_driver_id_fkey FOREIGN KEY (driver_id) REFERENCES public.profiles(id)
);
CREATE TABLE public.email_delivery_logs (
  ...
  html text,
  status text NOT NULL DEFAULT 'pending'::text CHECK (status = ANY (ARRAY['pending'::text, 'sent'::text, 'delivered'::text, 'bounced'::text, 'failed'::text, 'retried'::text])),
  bounce_type text,
  bounce_reason text,
  provider text NOT NULL DEFAULT 'brevo'::text,
...
CREATE TABLE public.admin_notifications (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  type text NOT NULL DEFAULT 'system'::text CHECK (type = ANY (ARRAY['new_shipment'::text, 'driver_application'::text, 'payment_failed'::text, 'support_ticket'::text, 'deletion_request'::text, 'email_failure'::text, 'assignment_update'::text, 'dispute'::text, 'system'::text])),
  title text NOT NULL,
  message text NOT NULL,
...
CREATE TABLE public.benji_events (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  ...
  CONSTRAINT benji_events_pkey PRIMARY KEY (id)
);
...
CREATE TABLE public.policy_violations (
  ...
  reason text,
  evidence text,
  severity text NOT NULL CHECK (severity = ANY (ARRAY['block'::text, 'warn'::text])),
  created_at timestamp with time zone NOT NULL DEFAULT now(),
  CONSTRAINT policy_violations_pkey PRIMARY KEY (id)
);
CREATE TABLE public.benji_traces (
  ...
  CONSTRAINT benji_traces_pkey PRIMARY KEY (trace_id)
);
CREATE TABLE public.benji_trace_steps (
  ...
  CONSTRAINT benji_trace_steps_trace_id_fkey FOREIGN KEY (trace_id) REFERENCES public.benji_traces(trace_id)
);
CREATE TABLE public.benji_pending_confirmations (
  ...
  created_at timestamp with time zone NOT NULL DEFAULT now(),
  CONSTRAINT benji_pending_confirmations_pkey PRIMARY KEY (trace_id)
);
CREATE TABLE public.benji_memories (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  user_id uuid NOT NULL,
  ...
  ttl_seconds integer,
  updated_at timestamp with time zone NOT NULL DEFAULT now(),
  CONSTRAINT benji_memories_pkey PRIMARY KEY (id)
);
CREATE TABLE public.benji_sessions (
  session_id text NOT NULL,
  user_id uuid NOT NULL,
  user_type text NOT NULL,
  channel text NOT NULL DEFAULT 'web'::text CHECK (channel = ANY (ARRAY['web'::text, 'mobile'::text, 'sms'::text, 'voice'::text])),
  phone_number text,
  ...
  expires_at timestamp with time zone NOT NULL,
  CONSTRAINT benji_sessions_pkey PRIMARY KEY (session_id),
  CONSTRAINT benji_sessions_user_id_fkey FOREIGN KEY (user_id) REFERENCES public.profiles(id)
);
CREATE TABLE public.support_escalations (
  ...
  CONSTRAINT support_escalations_user_id_fkey FOREIGN KEY (user_id) REFERENCES public.profiles(id)
);
CREATE TABLE public.quote_history (
  ...
  intelligent_price numeric,
  quoted_price numeric NOT NULL,
  base_rate_per_mile numeric NOT NULL,
  distance_band text NOT NULL,
  surge_multiplier numeric DEFAULT 1.00,
  delivery_type_multiplier numeric DEFAULT 1.00,
  fuel_adjustment_percent numeric DEFAULT 0.00,
  bulk_discount_percent numeric DEFAULT 0.00,
  minimum_applied boolean DEFAULT false,
  decision_maker text NOT NULL,
  benji_confidence_score integer,
  benji_reasoning text,
  override_reason text,
  competitor_prices jsonb,
  demand_indicator integer,
  supply_indicator integer,
  was_booked boolean DEFAULT false,
  booked_at timestamp with time zone,
  time_to_booking_ms bigint,
  shipment_id uuid,
  ...
  CONSTRAINT quote_history_user_id_fkey FOREIGN KEY (user_id) REFERENCES public.profiles(id),
  CONSTRAINT quote_history_shipment_id_fkey FOREIGN KEY (shipment_id) REFERENCES public.shipments(id)
);
CREATE TABLE public.pricing_events (
  ...
  source_service text DEFAULT 'pricing_service'::text,
  source_version text,
...
CREATE TABLE public.shipment_costs (
  ...
  driver_cost_per_mile numeric,
  insurance_cost_per_mile numeric,
  total_cost_per_mile numeric,
  finalized_at timestamp with time zone NOT NULL DEFAULT now(),
  notes text,
  created_by uuid,
  CONSTRAINT shipment_costs_pkey PRIMARY KEY (id),
  CONSTRAINT shipment_costs_shipment_id_fkey FOREIGN KEY (shipment_id) REFERENCES public.shipments(id),
  CONSTRAINT shipment_costs_created_by_fkey FOREIGN KEY (created_by) REFERENCES public.profiles(id)
);
CREATE TABLE public.route_analytics (
  ...
  min_quoted_price numeric,
  max_quoted_price numeric,
  median_quoted_price numeric,
  avg_booked_price numeric,
  avg_actual_cost numeric,
  avg_profit_margin numeric,
  avg_profit_margin_percent numeric,
  benji_usage_count integer DEFAULT 0,
  benji_conversion_rate numeric,
  baseline_conversion_rate numeric,
  competitor_avg_price numeric,
  our_price_vs_market_percent numeric,
  route_profitability_score integer,
  demand_score integer,
  supply_score integer,
  last_updated timestamp with time zone NOT NULL DEFAULT now(),
  sample_size integer,
  CONSTRAINT route_analytics_pkey PRIMARY KEY (id)
);