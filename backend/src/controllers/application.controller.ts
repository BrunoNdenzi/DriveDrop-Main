 ...
 */
export const applyForShipment = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (req.user.role !== 'driver') {
    throw createError('Only drivers can apply for shipments', 403, 'FORBIDDEN');
  }
  ...
  try {
    ...
    if (error) {
      logger.error('Error applying for shipment', { error, shipmentId, driverId: req.user.id });
    ...
    if (error instanceof Error) {
      if (error.message?.includes('This shipment has already been assigned')) {
        throw createError('This shipment already has a driver assigned', 400, 'ALREADY_ASSIGNED');
        ...
        throw createError('This shipment is not available for applications', 400, 'NOT_AVAILABLE');
      }
    }
 ...
 * @access Private (Driver)
 */
export const getDriverApplications = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (req.user.role !== 'driver') {
    throw createError('Only drivers can view their applications', 403, 'FORBIDDEN');
  }
  ...
  try {
    ...
    if (error) {
      logger.error('Error getting driver applications', { error, driverId: req.user.id, status });
...
export const updateApplicationStatus = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!status || !['pending', 'accepted', 'rejected', 'cancelled'].includes(status)) {
    throw createError('Invalid status. Must be pending, accepted, rejected, or cancelled', 400, 'INVALID_STATUS');
  }
  ...
  if (req.user.role === 'driver') {
    ...
    if (status !== 'cancelled') {
      throw createError('Drivers can only cancel their own applications', 403, 'FORBIDDEN');
    }
    ...
    if (application.driver_id !== req.user.id) {
      throw createError('You can only cancel your own applications', 403, 'FORBIDDEN');
    }
  } else if (req.user.role !== 'admin') {
    throw createError('Only admins and drivers can update application status', 403, 'FORBIDDEN');
  }
...
export const getAllApplications = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (error) {
      logger.error('Error getting all applications', { error, status, shipmentId });
      throw createError(error.message, 500, 'DATABASE_ERROR');
    }
