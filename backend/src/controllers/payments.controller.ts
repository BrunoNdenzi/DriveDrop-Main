  ...
  async (req: Request, res: Response) => {
    ...
    try {
      const paymentIntent = await stripeService.createPaymentIntent({
        amount: Math.round(amount * 0.2 * 100), // 20% initial payment in cents
        currency,
        ...
        metadata: {
          ...
          remainingAmount: Math.round(amount * 0.8 * 100).toString(),
        },
      });
      ...
      const { error: paymentError } = await supabase.from('payments').insert({
        ...
        initial_amount: Math.round(amount * 0.2 * 100), // 20% in cents
        remaining_amount: Math.round(amount * 0.8 * 100), // 80% in cents
      ...
      logger.info('Successfully created payment intent', {
        ...
        initialAmount: Math.round(amount * 0.2 * 100),
 ...
 * @access Private
 */
export const confirmPaymentIntent = asyncHandler(
  async (req: Request, res: Response) => {
    const { id } = req.params;
    ...
    if (!paymentMethodId) {
      logger.warn('Missing payment method ID in confirmation request', {
        paymentIntentId: id,
      });
      throw createError(
...
export const createSubscription = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    if (!customerId || !priceId) {
      throw createError(
        'Customer ID and price ID are required',
        400,
      ...
      successResponse({
        ...
        conversionRate:
          (analytics.successfulPayments /
            (analytics.successfulPayments + analytics.failedPayments || 1)) *
          100,
 ...
 * @access Private
 */
export const checkStripeStatus = asyncHandler(
 ...
 * @access Private
 */
export const checkRefundEligibility = asyncHandler(
  async (req: Request, res: Response) => {
...
export const processRefund = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (updateError) {
        logger.error('Failed to update payment status after refund', {
          error: updateError,
          paymentId,
        });
        throw createError(
 ...
 * @access Private
 */
export const createFinalPayment = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      const { data: paymentDetails, error: detailsError } = await supabase.rpc(
      ...
      if (detailsError || !paymentDetails || paymentDetails.length === 0) {
        throw createError(
          'Invalid parent payment or final payment already exists',
          400,
          'INVALID_PAYMENT'
        );
      }

      const { payment_intent_amount, payment_intent_description } =
        paymentDetails[0];
      ...
      if (paymentRecordError) {
        logger.error('Failed to create final payment record', {
      ...
      logger.error('Final payment creation failed', {
        error,
        parentPaymentId,
        shipmentId,
      });
