 ...
 * @access Private
 */
export const geocodeAddress = asyncHandler(
 ...
 * @access Private
 */
export const getDistanceMatrix = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    if (!Array.isArray(origins) || !Array.isArray(destinations)) {
      throw createError(
        'Origins and destinations must be arrays',
        400,
        'INVALID_FORMAT'
      );
    }
    ...
    if (origins.length === 0 || destinations.length === 0) {
      throw createError(
        'Origins and destinations cannot be empty',
        400,
        'EMPTY_ARRAYS'
      );
    }
 ...
 * @access Private
 */
export const calculateHaversineDistance = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    if (
      !googleMapsService.isValidCoordinates(lat1, lon1) ||
 ...
 */
export const optimizeRoute = asyncHandler(
  async (req: Request, res: Response) => {
 ...
 * @access Private
 */
export const validateCoordinates = asyncHandler(
  async (req: Request, res: Response) => {
    const { latitude, longitude } = req.body;
    ...
    const formatted = isValid
      ? googleMapsService.formatCoordinates(latitude, longitude)
      : null;
