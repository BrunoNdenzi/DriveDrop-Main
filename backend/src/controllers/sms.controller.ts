 ...
 */
export const sendShipmentNotification = asyncHandler(
  async (req: Request, res: Response) => {
    const { phoneNumber, shipmentId, status, trackingUrl } = req.body;

    if (!phoneNumber || !shipmentId || !status) {
      throw createError(
        'Phone number, shipment ID, and status are required',
        400,
        'MISSING_FIELDS'
      );
    }
 ...
 * @access Private (Admin only)
 */
export const sendDriverAssignmentNotification = asyncHandler(
  async (req: Request, res: Response) => {
    const { clientPhone, driverName, driverPhone, estimatedArrival } = req.body;

    if (!clientPhone || !driverName || !driverPhone) {
      throw createError(
        'Client phone, driver name, and driver phone are required',
        400,
        'MISSING_FIELDS'
      );
    }
 ...
 * @access Private (Driver or Admin)
 */
export const sendDeliveryConfirmation = asyncHandler(
  async (req: Request, res: Response) => {
    const { phoneNumber, shipmentId, deliveryTime, recipientName } = req.body;

    if (!phoneNumber || !shipmentId || !deliveryTime) {
      throw createError(
        'Phone number, shipment ID, and delivery time are required',
        400,
        'MISSING_FIELDS'
      );
    }
 ...
 * @access Private
 */
export const validatePhoneNumber = asyncHandler(
  async (req: Request, res: Response) => {
