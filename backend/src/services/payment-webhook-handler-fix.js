...
/**
 * Enhanced handlePaymentSucceeded method for stripe.service.ts
 * This version adds better error handling, shipment existence verification,
 * and uses upsert to prevent foreign key constraint violations
 */
...
/**
 * Helper function to create a payment notification
 * This assumes the implementation from your existing code
 */
async function createPaymentNotification(
  ...
  // Implementation would be based on your existing notification system
  ...
  const { error } = await supabaseAdmin
    ...
    .insert({
      user_id: userId,
      type: 'payment',
      message: `Your payment of $${(amount / 100).toFixed(2)} for shipment ${shipmentId} has been ${type}`,
      data: { shipmentId, paymentType: type, amount },
      read: false,
