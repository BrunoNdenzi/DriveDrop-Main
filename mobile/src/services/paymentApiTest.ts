...
export async function testPaymentApi() {
  try {
    ...
    if (authError) {
      ...
      return { success: false, error: 'Authentication error', details: authError };
    }
    ...
    try {
      ...
      return { success: false, error: 'Cannot reach API', details: healthError };
    }
    
    // 4. Test payment config endpoint
    try {
      console.log('Testing payment config endpoint...');
      const configResponse = await fetch(`${getApiUrl()}/api/v1/payments/config`, {
        method: 'GET'
      });
      console.log('Config Status:', configResponse.status, configResponse.statusText);
      ...
      return { success: false, error: 'Cannot fetch payment config', details: configError };
    }
    ...
    try {
      ...
      const intentResponse = await fetch(`${getApiUrl()}/api/v1/payments/create-intent`, {
        method: 'POST',
        headers: {
      ...
      console.log('Create Intent Status:', intentResponse.status, intentResponse.statusText);
      ...
      return { success: false, error: 'Cannot create payment intent', details: intentError };
    }
