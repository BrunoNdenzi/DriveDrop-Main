...
function checkEnvironmentVariables() {
  ...
  console.log('\nSummary:');
  if (missingRequired) {
    console.log('❌ Some required variables are missing. The app may not function correctly.');
    console.log('   Please create or update the .env file in the mobile directory.');
  } else if (missingOptional) {
    console.log('⚠️ All required variables are set, but some optional variables are missing.');
  ...
  if (apiUrl) {
    if (apiUrl.startsWith('http://localhost')) {
      ...
      console.log('   Consider using your computer\'s LAN IP address instead (e.g., http://192.168.1.x:3000)');
