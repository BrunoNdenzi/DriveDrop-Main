...
class AIService {
  ...
  /**
   * Benji V3 Chat — LLM-agent architecture.
   *
   * Sends the user message to the new V3 endpoint.
   * The backend maintains full conversation history server-side (keyed by sessionId).
   * No conversation history needs to be passed from the client.
   *
   * @param message   Current user message
   * @param sessionId Stable per-tab UUID (generated by useBenjiSession hook)
   * @param userType  Optional role hint (defaults to server-side derivation from JWT role)
   */
  async benjiV3Chat(
    message:   string,
    sessionId: string,
    userType?: 'client' | 'driver' | 'admin' | 'broker',
  ): Promise<{ response: string; sessionId: string; toolsUsed: string[]; latencyMs: number }> {
    const headers = await this.getHeaders()
    ...
    if (!response.ok) {
      throw new Error((data as any).error ?? 'Benji V3 chat request failed')
    }

    return data as { response: string; sessionId: string; toolsUsed: string[]; latencyMs: number }
  }
...
/** V3 response shape — matches backend V3ChatResponse. */
export interface BenjiV3Response {
  response:   string
  sessionId:  string
  toolsUsed:  string[]
  latencyMs:  number
