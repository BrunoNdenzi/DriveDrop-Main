...
const navigation = [
  { href: '/services/dump-trucks', label: 'Dump trucks', icon: Truck },
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 border-b border-white/15 bg-[#173436] text-white">
      <div className="mx-auto flex h-[68px] max-w-[1440px] items-center justify-between gap-4 px-4 sm:px-6 lg:px-8">
        ...
        <nav className="hidden h-full items-stretch xl:flex" aria-label="Services">
        ...
        <button type="button" className="grid h-9 w-9 place-items-center border border-white/25 xl:hidden" onClick={() => setIsMenuOpen(current => !current)} aria-label={isMenuOpen ? 'Close services menu' : 'Open services menu'} aria-expanded={isMenuOpen}>
          {isMenuOpen ? <X className="h-5 w-5" /> : <Menu className="h-5 w-5" />}
        </button>
      </div>

      {isMenuOpen && (
        <div className="border-t border-white/15 bg-[#173436] px-4 py-3 xl:hidden">
