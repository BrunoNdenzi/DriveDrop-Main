      ...
      <DialogContent className="max-w-5xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <div className="flex items-center gap-2 mb-2">
            ...
            <DialogTitle className="text-lg font-semibold">Choose Your Account Type</DialogTitle>
          </div>
        ...
        <div className="grid md:grid-cols-3 gap-4 py-6">
          {userRoles.map((roleOption) => (
            ...
            >
              <div className={`h-full ${roleOption.bgColor} border-2 ${roleOption.borderColor} rounded-md p-4 transition-all transform hover:scale-105`}>
                {/* Icon */}
                <div className={`w-14 h-14 ${roleOption.color} rounded-md flex items-center justify-center mb-4 group-hover:scale-110 transition-transform`}>
                ...
                <ul className="space-y-2 mb-6">
                  {roleOption.features.map((feature) => (
                    <li key={feature} className="flex items-center gap-2 text-sm text-gray-700">
                      <div className={`w-1.5 h-1.5 rounded-full ${roleOption.color} flex-shrink-0`} />
                      <span>{feature}</span>
                    </li>
                  ))}
                </ul>
                ...
                {/* CTA Button */}
                <Button 
                  className={`w-full ${roleOption.color} hover:opacity-90 text-white group-hover:translate-x-1 transition-transform`}
                  size="lg"
                >
