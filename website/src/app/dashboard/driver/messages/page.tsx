...
export default function DriverMessagesPage() {
  ...
  const loadConversations = useCallback(async () => {
    ...
    try {
      ...
      // Filter valid conversations for this driver
      const validConversations = (data || [])
        .filter(conv => 
          conv.shipment_id && 
          conv.client_id && 
          conv.driver_id &&
          conv.driver_id === profile.id
        )
  ...
  const navigateToChat = (conversation: Conversation) => {
    ...
    if (!conversation.client_id) {
      alert('Cannot identify the client for this shipment.')
      return
    }

    router.push(`/dashboard/driver/messages/${conversation.shipment_id}`)
  }
